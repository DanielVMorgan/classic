...
/* (c) 2022 Hypervariety Custom Software. All rights reserved. Packaged 2023-12-15 13:19:31 */ 
...
Attributes: contents,markChar,name,menuMessage|menuMsg
...
var contmatch;
while (contmatch=/[Â¬ï¾‚][\s]*(--(.*))?$/gi.exec(line)) {
...
if (handler) {
lineinfo.push({ indent: 0, error: "Unended handler " + handler.name });
commit_handler();
handler = null;
}
...
function warn() { if (false) console.warn(...arguments); }
...
error = "Can't understand â€˜" + line.trim() + "â€™";
...
flowResult.indent.forEach((depth,i)=>{
if (lineinfo.length > handler.line+1+i)	// a complex stack was returning one extra flowResult indent -- just throw it away, fix later
lineinfo[handler.line+1+i].indent = depth+1;
});
//	if (flowResult.indent.find((i)=>i!=0)) debugger;
if (flowResult.errors.length) log(flowResult.errors);
flowResult.errors.forEach((fre)=>{
if (lineinfo[handler.line+fre.line+1]) {
lineinfo[handler.line+fre.line+1].error = fre.error;
handler.code[fre.line] = ' // ' + handler.code[fre.line];
}
});
...
var yieldstatement = "yield(" + ((uniqueHandlerID << 16) + i+1) + "); ";
return "// " + "Â ".repeat(lineinfo[i].indent) + s.trim()
+ '\n'
+ " ".repeat(lineinfo[i].indent) + handler.code[i].substr(0,openbrace) + ((/*isclosebrace||*/options.dontYield) ? "" : yieldstatement) + handler.code[i].substr(openbrace)
+ '\n';
}).join('')
+ flowResult.autoclose
+ "yield(" + ((uniqueHandlerID << 16) + handler.script.length+1) + "); " + ' return "";'
...
{
var oldErrorsLength = result.errors.length;
var autoclose = adjuststack(matches[m][0].trim(), matches[m][1], (m==0), i);
var leftSideWasError = (oldErrorsLength != result.errors.length);
if (leftSideWasError) log('leftSideWasError', matches[m]);
//if (autoclose) { line = autoclose + ' ' + line; charadjust -= autoclose.length; }
if (autoclose) coroutine[i-1] += autoclose;
if (m==0) depth = flowstack.filter((fsf)=>fsf[1]=='?!').length;

autoclose = leftSideWasError ? '' : adjuststack(matches[m][2].trim(), matches[m][1], false, i);

line = line.substr(0,matches[m][3]-charadjust) + (leftSideWasError ? '' : matches[m][1]) + line.substr(matches[m][4]-charadjust);
charadjust += (matches[m][4]-matches[m][3]) - (leftSideWasError ? '' : matches[m][1]).length;
}
}
...
if (flowstack.length) { 	// unclosed flow routine
result.errors.push({ line: coroutine.length, error: 'missing ' + flowstack[0][0] });
result.autoclose += '}'.repeat(flowstack.length);
}

// this would mean a problem with the flow analysis (or means there's a brace in a quoted string, so we best not have this outside debugging)
...
warn('unsatisfied ' + top, flowstack);
}
}
...
warn('cannot execute ' + command, flowstack);
}
}
...
launch_scripteditor(debuggedhandler.xtalk.me, debuggedhandler.line + 1 + (debugloc & 0xFFFF), error, 0, true);
}
debug_step(debugloc)
{
var debuggedhandler = XTalk.DebugLocMap[debugloc >> 16];
...
if (debuggedhandler.errorProc)
{}
else if (true || XTalk.DebugScriptErrors) {
launch_scripteditor(debuggedhandler.xtalk.me, debuggedhandler.line + 1 + (debugloc & 0xFFFF), null, 0, true);
}
}
...
try
{
//var ms_per_slice = task.options.TemporaryLongSlices ? 1000 : 5, dnt = Date.now() + ms_per_slice;

var slice_start = Date.now();

run_loop: while (true)
{
var sev = document.getElementById('scripteditor') && scripteditor.visible;
...
debugloc = next.value;
if (sev) {
if (XTalk.DebugLocMap[debugloc >> 16] && scripteditor.target_element === XTalk.DebugLocMap[debugloc >> 16].xtalk.me) {
this.debug_step(debugloc);
yield 250;
}
}
}
...
debugger;	// shouldn't be here
}
if (Date.now() > (slice_start + (task.options.TemporaryLongSlices ? 1000 : 5)))
{
...
XTalk.cu = (what) => { throw new Error("Can't understand â€˜"+what+"â€™"); };
...
while (search) {
// if there's an exception here it tends to be because the coroutines had a syntax error.
...
{
// <rule> = literal | <nonterminal> | [zero or one] {one} [zero or more]* {one or more}*  => output using <nonterminal>
var tokenexp, match;

if (as_output)
tokenexp = /([\s]*)(<([A-Za-z0-9]+)>|([^\s\S])|([^\s\S])|([\|][\|]|[^\<\|\s]+|[^\|])|([\|]))/gy;
else
tokenexp = /([\s]*)(<([A-Za-z0-9]+)>|([\[\{])|(\]\*|\]\+|\]\-|\]|\}\*|\}\+|\})|([\|][\|]|[^\[\]\{\}\<\|\s]+|[^\|])|([\|]))/gy;
return subtree(rule);
...
=> ((cl,p)=>{ var pc = p.contents, r = xtalk_chunk(cl, pc, [], 'includeTrailing'); p.contents=[pc.substr(0,r[0]),pc.substr(r[1])].join(''); })([<chunkList>],<partWithContents>);
delete <chunkList> <variable>
=>  _.<variable> = ((r)=>[_.<variable>.substr(0,r[0]),_.<variable>.substr(r[1])].join(''))(xtalk_chunk([<chunkList>], _.<variable>, [], 'includeTrailing'));
#

<addSubtract> <expression> {to|from} <chunkList> <partWithContents>			=>  ((e,cl,p)=>{ var r = xtalk_chunk(cl, p.contents, [], true); p.contents=[p.contents.substr(0,r[0]),r[2],number(p.contents.substring(r[0],r[1])) <addSubtract> number(e),p.contents.substr(r[1])].join(''); })(<expression>,[<chunkList>],<partWithContents>);
<addSubtract> <expression> {to|from} <chunkList> <variable>				=>  ((e,cl)=>{ var p = String(_.<variable>), r = xtalk_chunk(cl, p, [], true); _.<variable> = [p.substr(0,r[0]),r[2],number(p.substring(r[0],r[1]))<addSubtract>number(e),p.substr(r[1])].join(''); })(<expression>,[<chunkList>]);
...
<multiplyDivide> <chunkList> <partWithContents> by <expression>			=>  ((e,cl,p)=>{ var r = xtalk_chunk(cl, p.contents, [], true); p.contents=[p.contents.substr(0,r[0]),r[2],number(p.contents.substring(r[0],r[1])) <multiplyDivide> number(e),p.contents.substr(r[1])].join(''); })(<expression>,[<chunkList>],<partWithContents>);
<multiplyDivide> <chunkList> <variable>	by <expression>			=>  _.<variable> = ((e,r)=>[_.<variable>.substr(0,r[0]),r[2],number(_.<variable>.substring(r[0],r[1])) <multiplyDivide> number(e),_.<variable>.substr(r[1])].join(''))(<expression>,xtalk_chunk([<chunkList>], _.<variable>, [], true));
...
do <factorsWithSpaces>										=>  yield*xtalk_do_coroutine(this, _, <factorsWithSpaces>);
...
{show|hide} [the] {groups|menubar|titlebar|scroll window|tool window|pattern window}						=>  null;
...
stop sound													=>  (yield*this.handlers.play('stop'));
...
choose [the] button tool									=>  (yield*this.handlers.choose('button'));
choose [the] field tool										=>  (yield*this.handlers.choose('field'));
...
drag from <expressionWithCommas> to <expressionWithCommas> with <expressionWithCommas>	=>  ((p,s,m)=>this.stackOf().clickDrag(p[0],p[1],s[0],s[1],m))(point([<expressionWithCommas>].join()),point([<expressionWithCommas>].join()),<expressionWithCommas>);
drag from <expressionWithCommas> to <expressionWithCommas> 	=>  ((p,s)=>this.stackOf().clickDrag(p[0],p[1],s[0],s[1]))(point([<expressionWithCommas>].join()),point([<expressionWithCommas>].join()));
...
find [chars|word|whole|string] [international] <expression> [in <buttonOrField>] [{of|in} marked cards]  => (yield*this.handlers.find(<expression>));

lock screen													=> (yield*this.handlers.setLockScreen(true));
unlock screen [with <extratext>]							=> (yield*this.handlers.setLockScreen(false));

{ flash | lock | unlock | dial | close window | convert | picture | start using | stop using | sort | reset {printing|paint} | delete <menuItem>  } [<extratext>]		=>	null;

reset menubar												=> this.stackOf().parentNode.resetMenubar();
...
=>	((e,cl,p)=>{ var c = String(p.<property>), r = xtalk_chunk(cl, c, [], true); p.<property> = [c.substr(0,r[0]),r[2],e,c.substr(r[1])].join(''); })(<expressionWithCommas>,[<chunkList>],<part>);

put <expressionWithCommas> into <chunkList> <partPossessive> <property> <possessiveChunkList> | set <chunkList> <partPossessive> <property> <possessiveChunkList> to <expressionWithCommas> =>	((e,p)=>{ var c = String(p.<property>), r = xtalk_chunk(cl, c, [<possessiveChunkList>], true); p.<property> = [c.substr(0,r[0]),r[2],e,c.substr(r[1])].join(''); })(<expressionWithCommas>,[<chunkList>], <partPossessive>);

put <expressionWithCommas> into <chunkList> <partPossessive> <property> | set <chunkList> <partPossessive> <property> to <expressionWithCommas> =>	((e,p)=>{ var c = String(p.<property>), r = xtalk_chunk(cl, c, [], true); p.<property> = [c.substr(0,r[0]),r[2],e,c.substr(r[1])].join(''); })(<expressionWithCommas>,[<chunkList>],<partPossessive>);

put <expressionWithCommas> into <chunkList> <partWithContents>	| set <chunkList> <partWithContents> to <expressionWithCommas> =>  ((e,cl,p)=>{ var c = String(p.contents), r = xtalk_chunk(cl, c, [], true); p.contents = [c.substr(0,r[0]),r[2],e,c.substr(r[1])].join(''); })(<expressionWithCommas>,[<chunkList>],<partWithContents>);

put <expressionWithCommas> into <chunkList> <variable> | set <chunkList> <variable> to <expressionWithCommas> => ((e,cl)=>{ var c = String(_.<variable>), r = xtalk_chunk(cl, c, [], true); _.<variable> = [c.substr(0,r[0]),r[2],e,c.substr(r[1])].join(''); })(<expressionWithCommas>, [<chunkList>]);
...
put <expressionWithCommas> into <menu> with {menuMessages|menuMsgs|menuMessage|menuMsg} <remainingTextNotElse>	=> ((menu,c,m)=>{ menu.contents = c; menu.menuMessages = m; })(<menu>,<expressionWithCommas>,<remainingTextNotElse>);
put <expressionWithCommas> into <menuItem> with {menuMessages|menuMsgs|menuMessage|menuMsg} <remainingTextNotElse>	=> ((menuItem,c,m)=>{ menuItem.contents = c; menuItem.menuMessage = m; })(<menuItem>,<expressionWithCommas>,<remainingTextNotElse>);
...
put <expression> before <chunkList> <partWithContents>		=>  ((e,cl,p)=>{ var r = xtalk_chunk(cl, p.contents, [], true); p.contents = [p.contents.substr(0,r[0]),r[2],e,p.contents.substr(r[0])].join(''); })(<expression>, [<chunkList>], <partWithContents>);
put <expression> before <chunkList> <variable>				=>  _.<variable> = ((e,r)=>[_.<variable>.substr(0,r[0]),r[2],e,_.<variable>.substr(r[0])].join(''))(<expression>,xtalk_chunk([<chunkList>], _.<variable>, [], true));
put <expressionWithCommas> before <menu> with {menuMessages|menuMsgs|menuMessage|menuMsg} <remainingTextNotElse>	=> ((menu,c,m)=>{ var oldMM = menu.menuMessages; menu.contents = c+(menu.contents.length?'\\n':'')+menu.contents; menu.menuMessages = m+'\\n'+oldMM; })(<menu>,<expressionWithCommas>,<remainingTextNotElse>);
put <expressionWithCommas> before <menu> 	=> ((menu,c,m)=>{ var oldMM = menu.menuMessages; menu.contents = c+(menu.contents.length?'\\n':'')+menu.contents; menu.menuMessages = m+'\\n'+oldMM; })(<menu>,<expressionWithCommas>,'');
put <expression> before <partWithContents> 					=>	((e,s)=>s.contents = [String(e), s.contents].join(''))(<expression>,<partWithContents>);
put <expression> before <variable> 							=>	_.<variable> = [<expression>,_.<variable>].join('');

put <expression> after <chunkList> <part>					=>  ((e,cl,p)=>{ var r = xtalk_chunk(cl, p.contents, [], true); p.contents = [p.contents.substr(0,r[1]),r[2],e,p.contents.substr(r[1])].join(''); })(<expression>, [<chunkList>], <part>);
put <expression> after <chunkList> <variable>				=>  _.<variable> = ((e,r)=>[_.<variable>.substr(0,r[1]),r[2],e,_.<variable>.substr(r[1])].join(''))(<expression>, xtalk_chunk([<chunkList>], _.<variable>, [], true));
put <expressionWithCommas> after <menu> with {menuMessages|menuMsgs|menuMessage|menuMsg} <remainingTextNotElse>	=> ((menu,c,m)=>{ var oldMM = menu.menuMessages; menu.contents += (menu.contents.length?'\\n':'')+c; menu.menuMessages = oldMM+'\\n'+m; })(<menu>,<expressionWithCommas>,<remainingTextNotElse>);
put <expressionWithCommas> after <menu>		=> ((menu,c,m)=>{ var oldMM = menu.menuMessages; menu.contents += (menu.contents.length?'\\n':'')+c; menu.menuMessages = oldMM+'\\n'+m; })(<menu>,<expressionWithCommas>,'');
put <expression> after <partWithContents>					=>	((e,s)=>s.contents += String(e))(<expression>,<partWithContents>);
put <expression> after <variable> 							=>	_.<variable> = _.<variable> += String(<expression>);
...
set [the] visible of [the] {message|msg} [box|window] to <expression> => if (window.messagebox) messagebox_setvisible(boolean(<expression>));

`);

XTalk.ImportGrammar('menuMessageList',`
<expression> <endOfScriptLine>
<remainingTextNotElse>
`);

XTalk.Support('remainingTextNotElse', /[^/S\n]*((?!else)[^\n])+/yi, '', {
postproduce(out)
{ out[0] = tickedString(out[0]); return true; }
});

function*xtalk_sort(input, delim, descAsc, sortStyle, byEach)
{
var sorties = input.split(delim);
for (var i = 0 ; i < sorties.length; i++)
sorties[i] = [i,sorties[i],yield*byEach(sorties[i])];
if (sortStyle.toLowerCase()=='numeric')
sorties.forEach(s=>(s[2]=parseFloat(s[2])));
sorties.sort((a,b)=>(a[2]===b[2])?0:(a[2]>b[2]?1:-1)*descAsc);
return sorties.map((a)=>a[1]).join(delim);
}

XTalk.ImportGrammar('sort',`
sort <itemsOfElseLinesOf> <partWithContents> <descendingElseAscending> <sortStyle> <byEach>
=>	<partWithContents>.contents = yield*xtalk_sort(<partWithContents>.contents,<itemsOfElseLinesOf>,<descendingElseAscending>,'<sortStyle>',<byEach>);
sort <itemsOfElseLinesOf> <variable> <descendingElseAscending> <sortStyle> <byEach>
=>	_.<variable> = yield*xtalk_sort(_.<variable>,<itemsOfElseLinesOf>,<descendingElseAscending>,'<sortStyle>',<byEach>);
`
);

XTalk.ImportGrammar('',`
<itemsOfElseLinesOf> = items {of|in} => ','
<itemsOfElseLinesOf> = [lines {of|in}] => '\\n'
<descendingElseAscending> = descending => -1
<descendingElseAscending> = [ascending] => 1
<sortStyle> = [ text | numeric | dateTime | international ]
<byEach> = by <expression> => function*(each){ _.each = each; return String(<expression>).toLowerCase(); }
<byEach> = [] => function*(str) { return String(str).toLowerCase(); }
`);

XTalk.ImportGrammar('go',`
go [to] [the] [stack] {home|"home"} [stack]							=>	goHome();
go [to] [the] [stack] {help|"help"} [stack]							=>	(yield*this.handlers.help());
| go [to] <card> [{of|in} [this|the] stack] <endOfScriptLine>			  				=>  (yield*this.stackOf().GoCoroutine(<card>));
...
<repeat> = repeat with <variable> {=|is} <expression> to <expression>
=> for (var <variable>$ = <expression>; (_.<variable>=<variable>$) <= <expression>; <variable>$++) /**/{/* +repeat?! */
<repeat> = repeat with <variable> {=|is} <expression> down to <expression>
=> for (var <variable>$ = <expression>; (_.<variable>=<variable>$) >= <expression>; <variable>$--) /**/{/* +repeat?! */
<repeat> = repeat with <factor> {=|is} <expression> to <expression>
=> for (var CTR$ = <factor>, VAR$ = <expression>; (_[CTR$]=VAR$) <= <expression>; VAR$++) /**/{/* +repeat?! */
<repeat> = repeat with <factor> {=|is} <expression> down to <expression>
=> for (var CTR$ = <factor>, VAR$ = <expression>; (_[CTR$]=VAR$) >= <expression>; VAR$--) /**/{/* +repeat?! */
...
<endUnknown> = end [<extratext>]				=> /* -unknown */{}/**/
`);

XTalk.ImportGrammar('command', `
<if> | <then> | <else> | <endif>
<repeat> | <exitrepeat> | <nextrepeat> | <endrepeat> | <endUnknown>
...
wait until <expression>										=>  { while (!boolean(<expression>)) yield({ timeout: 50 }); }
wait while <expression>										=>  { while (boolean(<expression>)) yield({ timeout: 50 }); }
...
menu <factor>												=> this.stackOf().parentNode.getMenu(<factor>,false)
`);

XTalk.ImportGrammar('menuItem',`
<ordinal> menuItem {of|in|from} <menu>						=> this.selectOf('title-bar-menuitem', 'ordinal', <ordinal>, <menu>, true)
menuItem <identOrExprOf> <menu>								=> this.selectOf('title-bar-menuitem', '', <identOrExprOf>, <menu>, true)
`);
...
return true;
var af,bf;
if (!isNaN(af=Number(a)) && !isNaN(bf=Number(b)) && /[\S]/.test(a) && /[\S]/.test(b))
return (af===bf);
...
| <expression> { â‰  | <> | is not [in]- | ï½­ } <expression>	   					 	=> !is_equal(<expression>,<expression>)
| <expression> { = | equals | is [less|more|in|not in]- } <expression>	 		=> is_equal(<expression>,<expression>)
<expression> { is {smaller|less} [than] or equal to | â‰¤ | <= | ï½² } <expression>		=> !is_more(<expression>,<expression>)
| <expression> { is {greater|more} [than] or equal to | â‰¥| >= | ï½³ } <expression>	=> !is_less(<expression>,<expression>)
...
[the] number of <chunkType> {of|in} <factor> 				=> xtalk_chunk(['<chunkType>', 'number', null], <factor>, [])
#
...
<chunkList> <factor> <possessiveChunkList>					=> xtalk_chunk([<chunkList>], <factor>, [<possessiveChunkList>])
<chunkList> <factor> 										=> xtalk_chunk([<chunkList>], <factor>, [])
<factor> <possessiveChunkList>								=> xtalk_chunk([], <factor>, [<possessiveChunkList>])
...
<messageCall> = {<message> | "<message>"} <expressionList>	=>	_.the_result = (yield*(this.handlers.<message>||XTalk.cu('<message>'))(<expressionList>));
...
XTalk.Support('identifier', /^[a-zA-ZÃ€-Å¾_][a-zA-ZÃ€-Å¾_0-9]*/i, '', {
...
XTalk.Support('string', /"[^"]+$/, '', {
postproduce(out) 	// string with no end quote was allowed in HC; it includes the first quote.
{ out[0] = '"\\' + out[0].replaceAll("\\", "\\\\") + '"'; return true; }
});
...
XTalk.Support('hcjunk', "\x03", "\"\\x0D\""); // HC reads raw enter ASCII as a return character
XTalk.Support('hcjunkpunct', /[`~#$%_.?;:\x01-\x09]/);
...
âˆš	2. can't have a list of possessivechunks left to right! Have to do them in reverse order, so maybe as recursive rule.
âˆš	3. Reversing things in the parser makes them evaluate in an unusual order. Maybe it doesn't matter.
âˆš	4. What about "first char of value's second word"? Can we roll it all together?
...
<ordinal> = any													=> 'any'
#<chunkCount> = [the] number of <chunkType> {of|in} <factor> 	=> xtalk_chunk(['<chunkType>', 'number', null], <factor>, [])
`, { show_to_user: true });

// first chunk of second chunk of value's fourth chunk's third chunk
function xtalk_chunk(ofchunks, value, posschunks, return_range)
{
value = String(value);
...
if (from==='last') {
if ((type == 'i' || type == 'l') && chunks.length > 1 && chunks[chunks.length-1][0]===chunks[chunks.length-1][1])
chunks.pop();
...
startvalue = chunks[Math.max(1, Math.min(chunks.length+1, from))-1];
if (!startvalue)
...
if (bodytool) bodytool.click(); else this.stackOf().tool = _.it;
end choose
...
size = number(size); gap = number(gap);
...
{
if (bits[i]=='1')
ctx.fillRect((size+gap)*(i%32), (size+gap)*Math.floor(i/32), size, size);
}
...
var DynamicHelperSymbol = Symbol('Ã˜');
...
newRange.setStart(range.startContainer, Math.min((range.startContainer.childNodes || range.startContainer.nodeValue).length, range.startOffset));
newRange.setEnd(range.endContainer, Math.min((range.endContainer.childNodes || range.endContainer.nodeValue).length, range.endOffset));
...
var xtalk_sound_channel = 1, xtalk_sound_channels = { };
function get_current_channel()
{
return xtalk_sound_channels[xtalk_sound_channel] || (xtalk_sound_channels[xtalk_sound_channel]={ stop_time: 0, nodes: [] });
}
...
delete audio_context.soundStopTimeout;
delete audio_context.soundStopACTime;

audio_context.connected_nodes.forEach((n)=>n.disconnect());
audio_context.connected_nodes = [];

sim.sound = '';

for (var sc in xtalk_sound_channels)
xtalk_sound_channels[sc].nodes.forEach((n)=>n.disconnect());
xtalk_sound_channels = { };
}
...
audio_context.connected_nodes.push(source);
...
//audio_context.connected_nodes.push(source);
get_current_channel().nodes.push(source);
}
...
//audio_context.connected_nodes.push(gainNode);
get_current_channel().nodes.push(gainNode);

var oscillator = audio_context.createOscillator();

oscillator.connect(gainNode);
oscillator.frequency.value = freq || 240;
//audio_context.connected_nodes.push(oscillator);
get_current_channel().nodes.push(oscillator);
...
if (channel.stop_time > audio_context.currentTime) t = (channel.stop_time - audio_context.currentTime)*1000;
...
channel.stop_time = audio_context.currentTime + t/1000;
clearTimeout(audio_context.soundStopTimeout);
...
body.fullscreen
{
--pointer-cursor: url(ui-icons/BrowseCursorLarge.png) 12 0;
}
@font-face {
...
@font-face {
font-family: 'Geneva';
xsrc: url('bitmap-fonts/bmpgeneva12/bmpgeneva12.ttf');
xsrc: url("bitmap-fonts/bitgeneva12-webfont.woff");
src: url("bitmap-fonts/bitgeneva12-webfont.otf");
font-weight: normal;
font-style: normal;
}
@font-face {
font-family: 'Dossier';
src: url("bitmap-fonts/Dossier-Regular.woff2");
font-weight: normal;
font-style: normal;
}
...
{ -webkit-transform: translate3d(0,0,0); -moz-transform: none; } 	/* this, finally, seems to fix the ghastly transform clipping bug in Safari */
...
/* using simple selector for card fields */
...
/* using the part() for card-part:shadow(background-part field-part)  */
stack-part[tool="Field" i] card-part::part(field) {
--special-field-tool-background: repeating-linear-gradient(var(--background, transparent), var(--background, transparent) calc(1.125em - 1px), #CCC 1.125em);
--special-field-tool-background-color-attribute: repeating-linear-gradient(var(--color-attribute, var(--background, transparent)), var(--color-attribute, var(--background, transparent)) calc(1.125em - 1px), #CCC 1.125em);
}
...
{
// the clickText just contains the closest word for the moment
var t, s = window.getSelection(), oldRange = s.rangeCount && s.getRangeAt(0), range = oldRange, cpft;
if (document.caretRangeFromPoint) {
range = document.caretRangeFromPoint(event.clientX, event.clientY);
}
else if (document.caretPositionFromPoint && (cpft=document.caretPositionFromPoint(event.clientX, event.clientY))) {
range = document.createRange();
range.setStart(cpft.offsetNode, cpft.offset);
range.setEnd(cpft.offsetNode, cpft.offset);
}
//console.log(range);
...
if (s.rangeCount) {
var bcr = s.getRangeAt(0).getBoundingClientRect();
//console.log(bcr, event.clientX, event.clientY,
//	bcr.left < event.clientX && event.clientX < bcr.right && bcr.top < event.clientY && event.clientY < bcr.bottom );
if (bcr.left < event.clientX && event.clientX < bcr.right && bcr.top < event.clientY && event.clientY < bcr.bottom)
sim.clickText = s.toString();
}

s.removeAllRanges();
if (oldRange) s.addRange(oldRange);
}
...
var lt = [(clickloc.x || clickX - layer.getBoundingClientRect().x) - clickRect[0],
(clickloc.y || clickY - layer.getBoundingClientRect().y) - clickRect[1],
clickRect[2] - (clickloc.x || clickX - layer.getBoundingClientRect().x),
clickRect[3] - (clickloc.y || clickY - layer.getBoundingClientRect().y)];
var m = Math.min(target.height/3, target.width/3, 12), newrect = clickRect, longpress = null, cancel_follow = false, cloned_target = null;
var hm = Math.min(target.width/3, 12), vm = Math.min(target.height/3, 12);
...
var mouseloc = stackpoint(event);
if (lt[0] <= hm && lt[1] <= vm)
newrect = [clickRect[0]+mouseloc.x-clickloc.x, clickRect[1]+mouseloc.y-clickloc.y, clickRect[2], clickRect[3]];
else if (lt[0] <= hm && lt[3] <= vm)
newrect = [clickRect[0]+mouseloc.x-clickloc.x, clickRect[1], clickRect[2], clickRect[3]+mouseloc.y-clickloc.y];
else if (lt[2] <= hm && lt[1] <= vm)
newrect = [clickRect[0], clickRect[1]+mouseloc.y-clickloc.y, clickRect[2]+mouseloc.x-clickloc.x, clickRect[3]];
else if (lt[2] <= hm && lt[3] <= vm)
newrect = [clickRect[0], clickRect[1],
Math.max(clickRect[0]+lt[2], clickRect[2]+mouseloc.x-clickloc.x),
Math.max(clickRect[1]+lt[3], clickRect[3]+mouseloc.y-clickloc.y)];
else {
...
{
var canvas = layer.canvas, context = layer.canvasContext2d, dpr = 2/*Math.round(window.devicePixelRatio||1)*/;
...
var onBlack = context.getImageData(startpoint.x*dpr,startpoint.y*dpr,1,1).data.join() == '0,0,0,255';
...
f.style = "margin: -4px; padding: 4px; margin-top: -0.8em; padding-top: -0.8em; outline: 1px dotted #8888; outline-offset: -0.25em; width: max-content;";
...
const range = document.createRange();
range.setStart(tn, 0);
var bcr1 = range.getBoundingClientRect();
if (f.textAlign=='right') bcr1.x = f.getBoundingClientRect().right - 8;
if (f.textAlign=='center') bcr1.x = (f.getBoundingClientRect().left+f.getBoundingClientRect().right)/2;
var sp1 =stackpoint({ clientX: bcr1.x, clientY: bcr1.y },true);
context.font = getComputedStyle(f).font;
context.textBaseline = 'top';
context.fillStyle = 'black';
context.textAlign = f.textAlign;
var lh = parseFloat(getComputedStyle(f).fontSize) * 1.12;
...
if (body.classList.contains('option') || optionclick || element.classList.contains('filled'))
{
context.closePath();
context.fill();
}
...
var startB=(colorLayer.data[(Math.floor(clickloc.y*dpr)*canvasWidth + Math.floor(clickloc.x*dpr))*4+2]);
var startA=(colorLayer.data[(Math.floor(clickloc.y*dpr)*canvasWidth + Math.floor(clickloc.x*dpr))*4+3]);
console.log([clickloc.x,clickloc.y],[startR,startG,startB,startA]);
...
if (cefae && cefae.nodeName != 'FIELD-PART')
cefae = cefae.parentNode.host;
if (cefae && cefae.nodeName != 'FIELD-PART')
cefae = null;
...
{
// supports the "click at P [with modifiers]" and "drag from P to P [with modifiers]"
var oldShift = body.classList.contains('shift'), oldOption = body.classList.contains('option'), oldCommand = body.classList.contains('command');
body.classList.toggle('shift', /shiftKey/i.test(modifiers));
body.classList.toggle('option', /optionKey/i.test(modifiers));
body.classList.toggle('command', /commandKey/i.test(modifiers));
...
shiftKey: /shiftKey/i.test(modifiers), altKey: /optionKey/i.test(modifiers), metaKey: /commandKey/i.test(modifiers) });
...
body.classList.toggle('shift', oldShift);
body.classList.toggle('option', oldOption);
body.classList.toggle('command', oldCommand);
},
...
bkgnd.cloned_node.setAttribute('part', "background");
...
var cards = Array.from(element.qsa('card-part')).reverse();
element.card = cards[cards.findIndex((c)=>c===target)+1] || cards[0];
}
...
{
var canvas, context2d, dpr = 2/*Math.round(window.devicePixelRatio||1)*/;
...
card-part > div[slot] { outline: none; width: 100%; }
...
if (!lock && element.classList.contains('script-indent')) try { template.qs('#inner').contentEditable = 'plaintext-only'; } catch(e) {}
}
(ceTarget || element).contentEditable = !lock;
if (!lock && element.classList.contains('script-indent')) try { (ceTarget || element).contentEditable = 'plaintext-only'; } catch(e) {}
}
...
if ( (/^(https|http|file)[\:][\/][\/]/y).test(value) || (/^(data)[\:](image[\/])/y).test(value))
...
{
if (value == -1)
data64 = (stack.localPICTs[Object.keys(stack.localPICTs).find((p)=>(stack.localPICTs[p].name||'').toLowerCase()==String(element.name).toLowerCase())] || {}).bitmap;
else
...
function setnow()
{
if (/*isNaN(value) || !Number.isInteger(Number(value))*/ /[\.\/]/.test(value))
template.qs('#icon img').src = value+(/[.][A-Za-z0-9]+$/.test(value)?'':'.png');
else
template.qs('#icon img').src = 'png-icons/icon_'+String(value).toLowerCase()+'.png';
...
set name(value) { caption.dataset.name = element.name; if (element.icon==-1) seticon(element.icon); },
set color(value) { setaddcolor(); },
set bevel(value) { setaddcolor(); },
set icon(value) {
seticon(value);
...
if (element.has_false_innerHTML && it=='Â ') it = '';
return it.substring(0,(it[it.length-1]=='\n') ? it.length-1 : it.length);
},
set contents(value) {
value = String(value);
element.innerText = (element.has_false_innerHTML=!value) ? 'Â ' : value; /* needed for in-text sel */
fillselect();
},
get selectedLine() {
if (element.owner)
...
font: var(--caption-wrapped-font, inherit); line-height: 0.8em; xletter-spacing: -0.45px; text-rendering: geometricPrecision; }
...
var itemNames = String(value).split(/[,\n]/);
element.innerHTML = itemNames.map((i)=>"<title-bar-menuitem>"+ContentEditableFix.HTMLEncode(i)+"</title-bar-menuitem>").join('');
element.childNodes.forEach((n,i)=>n.name = itemNames[i]);
},
get contents() {
...
get menuMessages() {
return Array.from(element.qsa('title-bar-menuitem')).map((tbmi)=>tbmi.menuMessage || '').join('\n');
}
};
}
...
:host > span::after { content: " â–¼"; color: gray; font-size: 0.75em; }
...
{ return element.name },
set markChar(value)
{ template.qs('#markChar').innerText = String(value)[0] || 'Â '; },
get markChar()
{ return template.qs('#markChar').innerText.trim(); },
set checkMark(value)
{ template.qs('#markChar').innerText = boolean(value) ? 'âˆš' : ' '; },
get checkMark()
{ return template.qs('#markChar').innerText=='âˆš'; }
};
}
ElementTemplate.Create("title-bar-menuitem", "contents,markChar,name,menuMessage|menuMsg", "",`
...
var s = /*fullscreen ? Math.min(s,1) :*/ Math.min(/*(xshrink < 0.5) ? 1 :*/ xshrink, (yshrink < 0.7) ? 1 : yshrink);
...
(windowInnerHeight*visualViewport.scale - document.getElementById('toolbar').offsetHeight/*stackcontainertd.offsetTop*/) /*+window.scrollY*/
/*
...
if (s > 1 && matchMedia('(pointer:fine)').matches && !fullscreen && body.classList.contains('dont-magnify-stacks'))
s = 1;
...
((!fullscreen ? 'translate(0,0)' : 'translate(' + Math.floor((size[0]-md.shadowRoot.qs('#dialog').offsetWidth*s)/2) +'px, 0px)'))
+ ((s!=1) ? ' scale('+s+') ' : ''));
//console.log((12 / s)+'px Chicago');
md.style.setProperty('--modal-dialog-titlebar-font',
matchMedia('(pointer:coarse)').matches
? '18px Chicago'	// if we shrink the window it needs the zoom box to be bigger up to a point
: '12px Chicago');
...
createIfMissing = createIfMissing || "apple,file,edit,go,objects".split(',').includes(name.toLowerCase());
...
resetMenubar() {
element.menubar.innerHTML = '';
}
};
}
ElementTemplate.Create("modal-dialog", "name,visible,x,y", "",`<div id="dialog"><div id="titlebar"><div class="boxmargin left" style="flex: 0 1 2em;"><div id="closebox"></div></div><div style="background-position: top right;"></div><span id="title" style="position: relative;"></span><span id="flashmessage" style="color: green;"></span><span id="menubar" style="margin-top: -2px;"></span><div> </div><div class="boxmargin right" style="flex: 0 1 2em; "><div id="zoombox"></div></div></div><div id="content" part="content"><span id="cover" onpointerdown="if (typeof sim != 'undefined') sim.mouse='down';" onpointerup="if (typeof sim != 'undefined') sim.mouse='up';"><div class="answer"><button-part type="standard" visible="false" name="Extrabutton" onclick=""></button-part><button-part class="reload" type="default" name="Reload â†º" onclick="this.getRootNode().host.performUpdateReload(); this.getRootNode().host.classList.remove('loading','offer-update');"></button-part> <button-part type="standard" class="cancel" name="Cancel" onclick="this.getRootNode().host.classList.remove('loading','offer-update');"></button-part> </div></span><slot></slot><!--div style="clear:both;"></div!--></div></div><style>
...
background: white; padding: 0.5em 1em; border-radius: 0.5em; xborder: thin solid gray;
...
:host(.loading:not(.offer-update)) #cover .answer:before
{ content: "";  background-image: url(ui-icons/watch.png); background-repeat: no-repeat; display: inline-block; width: 16px; height: 16px; }
...
{ content: "Wait... (âŒ˜ .)"; }	/* should change this to fluid [data-wait-message] or something. = wait 3 seconds, Loading..., etc */

:host(.loading.offer-update) #cover > .answer {
xbox-shadow: 0px 0px 4px gray; border: thin solid gray;
}
...
:host(.static) #dialog {
xbox-shadow: 1px 1px 0px var(--dialog-border);
}
:host(.current.static) #dialog {
box-shadow: 1px 1px 0px var(--dialog-border), 0px 2px 5px #CCC;
}
:host(.ybottom) #dialog {
position: absolute;
top: auto;
...
#titlebar {
text-align: center;
display: flex; padding: 3px 2px 0px 2px;
...
background: linear-gradient(to bottom, gray 0%, gray 1px, transparent 1px, transparent 100%);
background-size: 100% 3px;
background-clip: content-box;
}
:host(.dotted:not(.inactive)) #titlebar > div {
background-image: radial-gradient(#669 0.75px, #BBB1 0);
...
:host(.closebox) #closebox, :host(.zoombox) #zoombox {
display: inline-block; width: 1em; height: 1em; border: 1px solid currentColor;
vertical-align: top; box-sizing: border-box; position: relative;
background: var(--dark-mode-softwhite, white);
outline: 2px solid var(--dark-mode-softwhite, white);
}
:host(.zoombox) #zoombox:before {
position: absolute; content: ""; left: -1px; top: -1px; width: 0.6em; height: 0.6em; border: 1px solid currentColor; box-sizing: border-box;
}
:host(.zoombox) #zoombox:active:hover:before {
left: -2px; top: -2px; border-width: 2px; width: calc(0.6em + 0.5px); height: calc(0.6em + 0.5px);
}

:host(.closebox) #closebox:after, :host(.zoombox) #zoombox:after {
...
:host(.closebox) #closebox:active:hover, :host(.zoombox) #zoombox:active:hover {
...
xcontent: '\\A0ðŸŒ\\A0' attr(data-name) '\\A0';
...
document.write(`<modal-dialog id="answerdialog" name="" style="--modal-dialog-titlebar-font:  0.5em system-ui;" tabindex="0">
		<table><tbody><tr><td>
...
document.write(`<modal-dialog id="askdialog" name="" style="--modal-dialog-titlebar-font:  0.5em system-ui;" tabindex="0">
		<table><tbody><tr><td>
			<div id="text" style="width: 350px; margin-bottom: 0.5em">Hey, could you lorem ipso sum?</div>
			<field-part id="result" type="rectangle" style="margin: 0px;" part="field" contenteditable="true"><div>Yes</div></field-part>
		</td></tr>
			<tr><td>
				<div class="flex-row" onclick="if (event.target.matches('button-part')) {
